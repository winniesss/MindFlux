
import { GoogleGenAI, Type } from "@google/genai";
import { AnalysisResult, Weight, Language, Thought, ThoughtStatus } from "../types";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

const getAnalysisSchema = (lang: Language) => ({
  type: Type.OBJECT,
  properties: {
    category: {
      type: Type.STRING,
      enum: ["LET_THEM", "LET_ME"],
      description: "Classify if this is outside control (LET_THEM/Smoke) or an actionable task (LET_ME/Solid)."
    },
    weight: {
      type: Type.STRING,
      enum: ["URGENT", "IMPORTANT", "CASUAL"],
    },
    reasoning: {
      type: Type.STRING,
      description: `Friendly explanation in ${lang === 'zh' ? 'Chinese' : 'English'}.`
    },
    reframing: {
      type: Type.STRING,
      description: `Supportive reframing in ${lang === 'zh' ? 'Chinese' : 'English'}.`
    },
    stoicQuote: {
      type: Type.STRING,
      description: `Comforting insight in ${lang === 'zh' ? 'Chinese' : 'English'} (max 15 words).`
    },
    subTasks: {
      type: Type.ARRAY,
      items: { type: Type.STRING },
      description: `1-3 gentle steps in ${lang === 'zh' ? 'Chinese' : 'English'}.`
    },
    timeEstimate: {
      type: Type.STRING,
      description: "Time estimate (e.g. '45m')."
    },
    suggestedSlot: {
      type: Type.STRING,
      description: `Suggested time of day (e.g. '10:30' or 'Evening'). Use ${lang === 'zh' ? 'Chinese' : 'English'}.`
    }
  },
  required: ["category", "reasoning", "stoicQuote", "suggestedSlot"]
});

const splitChaosSchema = {
  type: Type.ARRAY,
  items: {
    type: Type.OBJECT,
    properties: {
      text: { type: Type.STRING, description: "One distinct thought or task." }
    },
    required: ["text"]
  }
};

export const splitChaos = async (content: string, lang: Language): Promise<Partial<Thought>[]> => {
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `Deconstruct: "${content}". Use language: ${lang === 'zh' ? 'Chinese' : 'English'}.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: splitChaosSchema,
        thinkingConfig: { thinkingBudget: 0 },
        systemInstruction: `You are an empathetic listener. Break down human input into clear, separate thoughts. Use ${lang}.`
      }
    });

    const results = JSON.parse(response.text || "[]");
    return results.map((r: any) => ({
      content: r.text,
      status: ThoughtStatus.UNSORTED
    }));
  } catch (error: any) {
    const parts = content.split(/[，。？！；,\.?!;]\s*/).filter(p => p.trim().length > 0);
    return parts.map(p => ({ content: p.trim(), status: ThoughtStatus.UNSORTED }));
  }
};

export const analyzeThought = async (
  content: string, 
  lang: Language
): Promise<AnalysisResult> => {
  const now = new Date();
  const currentTimeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
  
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `Analyze: "${content}". Time: ${currentTimeStr}. Lang: ${lang}.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: getAnalysisSchema(lang),
        thinkingConfig: { thinkingBudget: 0 },
        systemInstruction: `You are a warm, stoic mentor. Classify if a thought is actionable or simply a noise to be accepted. Be extremely supportive in ${lang}.`
      }
    });

    return JSON.parse(response.text || "{}") as AnalysisResult;
  } catch (error: any) {
    return {
      category: 'LET_ME',
      reasoning: lang === 'zh' ? "行动是治疗焦虑的良药。" : "Action is the antidote to anxiety.",
      stoicQuote: lang === 'zh' ? "迈出第一步" : "Take the first step",
      suggestedSlot: "Now"
    };
  }
};
